...
function LogoModel({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  useFrame((state) => {
    if (modelRef.current) {
      ...
      const idleSpin = state.clock.elapsedTime * 0.2
      ...
      modelRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.5) * 0.1 + 0.1
      modelRef.current.rotation.z = Math.cos(state.clock.elapsedTime * 0.5) * 0.05
    }
  })
...
// Removed preload so it doesn't block the initial app boot
// The model will load when the user reaches the FAQ section
